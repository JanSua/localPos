...
# quickstart.sh inside WSL2 to install and start nodedr-pos. It also installs
# the native Windows Docker CLI (winget: Docker.DockerCLI) so `docker` exists
# as a plain PowerShell command too, alongside the one running inside WSL2.
#
...
$distroName = "Ubuntu"
$repoDir = "nodedr-pos"
$linuxQuickstartUrl = "https://raw.githubusercontent.com/Raktim94/nodedr-pos/master/scripts/quickstart.sh"
...
if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
    if (Get-Command winget -ErrorAction SilentlyContinue) {
        ...
        Write-Host "winget isn't available, so the native Windows 'docker' command will be skipped — nodedr-pos will still install and run fully inside WSL2." -ForegroundColor Yellow
    }
}

Write-Host "==> Installing and starting nodedr-pos inside WSL2 (Docker Engine and git are installed automatically in there if needed)..."
wsl.exe -d $distroName -u root -- bash -lc "curl -fsSL $linuxQuickstartUrl | bash"

Write-Host ""
Write-Host "nodedr-pos is up and running." -ForegroundColor Green
